...
const db = mysql.createConnection({
  ...
  database: 'iot_tiendat' // Tên database bạn muốn lưu
});
...
const saveSensorData = (temperature, humidity, light, time) => {
  const sql = `INSERT INTO datasensor (temperature, humidity, light, time) VALUES (?, ?, ?, ?)`;
...
const saveActionHistory = (device, status, time) => {
  const sql = `INSERT INTO history (device, status, time) VALUES (?, ?, ?)`;

  db.query(sql, [device, status, time], (err, result) => {
    if (err) {
      console.error('Lỗi khi chèn dữ liệu vào history:', err);
    } else {
